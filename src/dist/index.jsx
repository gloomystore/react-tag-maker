...
function Tag({ text, remove, disabled, className }) {
    ...
    return (<span className={classNames('gloomy-tag--tag', className)}>
      <span>{text}</span>
      {!disabled && (<button type='button' onClick={handleClick} aria-label={`remove ${text}`}>
          &#10005;
        </button>)}
    </span>);
}
let timeout;
export default function GloomyTags({ name, placeHolder, state, setState, onChange, onBlur, separators = [], disableBackspaceRemove = false, onExisting, onRemoved, disabled = false, isEditOnRemove = false, beforeAddValidate, onKeyUp, classNames, style, throttleTime, }) {
    ...
    const handleKeyDown = (event) => {
        ...
        if (inputValue && (separators.includes(key) || ENTER_KEYS.includes(key))) {
            ...
            // 쓰로틀 상태에서는 안함
            if (!throttle)
                setThrottle(setTimeout(() => { setThrottle(0); }, throttleTime ?? 300));
            else
                return console.warn('react-tag-maker throttling!');
    ...
    const cursorToInput = useCallback((e) => {
        if (!inputRef.current || e.target.className !== 'gloomy-tag--container')
            return;
        inputRef.current.focus();
    }, []);
    return (<div className='gloomy-tag--container' onClick={cursorToInput} style={style}>
      {focusArray.map((item) => (<Tag key={item.tag} className={`${classNames?.tag} ${item.focus && 'gloomy-tag--focus'}`} text={item.tag} remove={handleTagRemove} disabled={disabled}/>))}
      <input className={`gloomy-tag--input ${classNames?.input}`} type='text' name={name} placeholder={placeHolder} onKeyDown={handleKeyDown} onBlur={onBlur} disabled={disabled} onKeyUp={onKeyUp} ref={inputRef}/>
